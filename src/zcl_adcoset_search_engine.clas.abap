...
CLASS zcl_adcoset_search_engine DEFINITION
  ...
  PRIVATE SECTION.
    CLASS-DATA instance TYPE REF TO zcl_adcoset_search_engine.

    METHODS validate_matchers
      IMPORTING
...
CLASS zcl_adcoset_search_engine IMPLEMENTATION.
  ...
  METHOD run_code_search_arfc.
    ...
    output = VALUE #( result_objects         = query->get_results( )
  ...
  METHOD search_code.
    ...
    validate_matchers( matcher_type = search_config-matcher_type
    ...
    result = VALUE #( results                = query->get_results( )
                      messages               = zcl_adcoset_log=>get_messages( )

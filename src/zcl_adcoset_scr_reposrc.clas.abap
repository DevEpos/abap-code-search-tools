...
CLASS zcl_adcoset_scr_reposrc DEFINITION
  ...
  PRIVATE SECTION.
    CONSTANTS c_default_comment_regex TYPE string VALUE '^(\*|\s*")'.
    CONSTANTS c_bdef_comment_regex TYPE string VALUE '^\s*(//|/\*)'.
...
CLASS zcl_adcoset_scr_reposrc IMPLEMENTATION.
  ...
  METHOD zif_adcoset_src_code_reader~get_source_code.
    ...
    IF is_multiline = abap_true.
      zcl_adcoset_string_util=>transform_to_string( EXPORTING source_table = source
                                                              line_feed    = line_feed

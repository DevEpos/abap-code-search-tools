...
INTERFACE zif_adcoset_search_scope
  ...
  METHODS next_package
    RETURNING VALUE(result) TYPE zif_adcoset_ty_global=>ty_tadir_objects_deep.
  ...
  METHODS configure_package_size DEFAULT IGNORE
    IMPORTING max_objects    TYPE i OPTIONAL
  ...
  METHODS get_scope_ranges DEFAULT IGNORE
    RETURNING VALUE(result) TYPE zif_adcoset_ty_global=>ty_search_scope_ranges.
ENDINTERFACE.

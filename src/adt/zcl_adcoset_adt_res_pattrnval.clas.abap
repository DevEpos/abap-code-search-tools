...
CLASS zcl_adcoset_adt_res_pattrnval DEFINITION
  ...
  PRIVATE SECTION.
    ...
    DATA is_regex TYPE abap_bool.
    DATA pattern_configs TYPE zif_adcoset_ty_global=>ty_patterns.

    METHODS collect_patterns
      IMPORTING
        patterns TYPE string.

    METHODS get_uri_params
      IMPORTING
    ...
    METHODS validate_regex_patterns
      RAISING
        cx_adt_rest.

    METHODS validate_sequential_patterns
      RAISING
        cx_adt_rest.
ENDCLASS.
...
CLASS zcl_adcoset_adt_res_pattrnval IMPLEMENTATION.
  METHOD post.
    ...
    IF patterns IS INITIAL.
      RAISE EXCEPTION TYPE zcx_adcoset_adt_rest
        EXPORTING text = |No pattern supplied|.
    ENDIF.
  ...
  METHOD collect_patterns.
    ...
    pattern_configs = VALUE zif_adcoset_ty_global=>ty_patterns( FOR <pattern> IN separate_patterns
                                                                ( content = <pattern> ) ).
  ENDMETHOD.
  ...
  METHOD validate_regex_patterns.
    DATA(matcher_type) = COND #(
      WHEN zcl_adcoset_pcre_util=>is_pcre_supported( )
      THEN zif_adcoset_c_global=>c_matcher_type-pcre
      ELSE zif_adcoset_c_global=>c_matcher_type-posix_regex ).

    LOOP AT pattern_configs ASSIGNING FIELD-SYMBOL(<pattern>).
      TRY.
          zcl_adcoset_matcher_factory=>create_matcher( type    = matcher_type
                                                       pattern = <pattern> ).
        CATCH cx_sy_regex INTO DATA(regex_error).
          RAISE EXCEPTION TYPE zcx_adcoset_adt_rest
            EXPORTING text = |{ regex_error->get_text( ) }: "{ <pattern>-content }"|.
  ...
  METHOD validate_sequential_patterns.
    TRY.
      ...
      CATCH zcx_adcoset_static_error INTO DATA(parsing_error).
        RAISE EXCEPTION TYPE zcx_adcoset_adt_rest
          EXPORTING previous = parsing_error.
    ENDTRY.
  ENDMETHOD.
ENDCLASS.

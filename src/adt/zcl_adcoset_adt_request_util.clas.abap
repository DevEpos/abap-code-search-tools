...
CLASS zcl_adcoset_adt_request_util DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get_query_parameter_values
      IMPORTING
    ...
    CLASS-METHODS get_query_parameter
      IMPORTING
        param_name    TYPE string
    ...
    "! <p class="shorttext synchronized">Retrieve value of integer request parameter</p>
    CLASS-METHODS get_integer_query_parameter
      IMPORTING
        param_name    TYPE string
    ...
    CLASS-METHODS get_boolean_query_parameter
      IMPORTING
        param_name    TYPE string
    ...
    "! <p class="shorttext synchronized">Retrieves UUID request parameter</p>
    CLASS-METHODS get_uuid_uri_query_parameter
      IMPORTING
        param_name    TYPE string
        mandatory     TYPE abap_bool OPTIONAL
        !request      TYPE REF TO if_adt_rest_request
...
CLASS zcl_adcoset_adt_request_util IMPLEMENTATION.
  ...
  METHOD get_integer_query_parameter.
    DATA(param_value) = get_query_parameter( param_name = param_name
                                             mandatory  = mandatory
                                             request    = request ).
    ...
    ELSEIF param_value CN '0123456789'.
      RAISE EXCEPTION TYPE zcx_adcoset_adt_rest
        EXPORTING text = |Non Integer value '{ param_value }' was passed to Parameter '{ param_name }'|.
    ELSE.
      result = param_value.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD get_boolean_query_parameter.
    TRY.
        DATA(value) = get_query_parameter( param_name = param_name
                                           request    = request ).
  ...
  METHOD get_uuid_uri_query_parameter.
    DATA(uuid_c36_string) = get_query_parameter( param_name = param_name
                                                 mandatory  = mandatory
                                                 request    = request ).

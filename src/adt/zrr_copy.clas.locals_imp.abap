...
CLASS lcl_search_query IMPLEMENTATION.
  ...
  METHOD run.
    ...
    TRY.
        ...
        result = NEW lcl_result_converter( raw_result             = search_result
  ...
  METHOD parse_parameters.
    ...
    settings-search_scope = VALUE #( max_objects    = zcl_adcoset_adt_request_util=>get_integer_query_parameter(
  ...
  METHOD get_patterns.
    settings-patterns = VALUE #( FOR pattern IN  zcl_adcoset_adt_request_util=>get_query_parameter_values(
  ...
  METHOD get_persisted_settings.
    ...
    settings-pcre_settings       = VALUE #( extended_mode_disabled   = server_settings-pcre_ext_mode_disabled
                                            single_line_mode_enabled = server_settings-pcre_single_line_enabled ).
    settings-parallel_processing = VALUE #( enabled      = server_settings-parallel_enabled
    ...
    LOOP AT scopes INTO DATA(scope).
      CASE scope.
        ...
        WHEN zif_adcoset_c_global=>c_class_include_id-all.
          <class_incl_flags> = VALUE #( public_section    = abap_true
    ...
    LOOP AT scopes INTO DATA(scope).
      CASE scope.
        ...
        WHEN zif_adcoset_c_global=>c_fugr_include_id-all.
          <fugr_incl_flags> = VALUE #( function     = abap_true
  ...
  METHOD add_packages_to_adt_result.
    LOOP AT packages ASSIGNING FIELD-SYMBOL(<package>).
      ...
      APPEND VALUE #( uri             = <package>-uri
                      adt_main_object = VALUE #( type = c_adt_wb_object_type-package
  ...
  METHOD create_match_object_ref.
    CASE object_info-type.
      ...
      WHEN zif_adcoset_c_global=>c_source_code_type-class OR
        ...
        result = adt_obj_factory->get_object_ref_for_include( main_program      = object_info-name

...
CLASS zcl_adcoset_extensions_util DEFINITION
  ...
  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Checks if ABAP Tags feature is installed</p>
    CLASS-METHODS is_abap_tags_available
      RETURNING
        VALUE(result) TYPE abap_bool.

    "! <p class="shorttext synchronized">Retrieves current table with stored tagged objects</p>
    CLASS-METHODS get_current_tgobj_table
      RETURNING
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS is_tgobj_table_available
      IMPORTING
        tabname       TYPE tabname
      RETURNING
        VALUE(result) TYPE abap_bool.

    CLASS-METHODS test_field_existence
      IMPORTING
...
CLASS zcl_adcoset_extensions_util IMPLEMENTATION.
  ...
  METHOD is_tgobj_table_available.
    ...
    DATA table_fields TYPE ty_table_fields.

    CALL FUNCTION 'DDIF_TABL_GET'
      EXPORTING  name          = tabname
      IMPORTING  dd02v_wa      = table_def
      TABLES     dd03p_tab     = table_fields
      EXCEPTIONS illegal_input = 1
                 OTHERS        = 2.
    IF     sy-subrc            = 0
       AND table_def-tabclass  = 'TRANSP'
       AND table_fields       IS NOT INITIAL
       AND test_field_existence(
               fields         = table_fields
               fields_to_test = VALUE #( ( fieldname = 'OBJECT_NAME' rollname = 'SOBJ_NAME' )
  ...
  METHOD test_field_existence.
    ...
    LOOP AT fields_to_test ASSIGNING FIELD-SYMBOL(<field_to_test>).
      IF NOT line_exists( fields[ fieldname = <field_to_test>-fieldname

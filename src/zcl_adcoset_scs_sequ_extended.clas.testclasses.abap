...
CLASS lcl_test_helper DEFINITION CREATE PRIVATE.
  PUBLIC SECTION.
    CLASS-METHODS create_substring_matchers
      IMPORTING
...
CLASS lcl_test_helper IMPLEMENTATION.
  ...
  METHOD get_source.
    ...
    source_content = VALUE #( ( |  METHOD find_next_full_match.| )
                              ( || )
    ...
    result = NEW zcl_adcoset_source_code( source        = source_content
                                          comment_regex = '^(\*|\s*")' ).
  ENDMETHOD.
ENDCLASS.
...
CLASS ltcl_abap_unit DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS test_match_found1    FOR TESTING.
    METHODS test_match_found2    FOR TESTING.
    METHODS test_no_match_found1 FOR TESTING.
    METHODS test_no_match_found2 FOR TESTING.
    METHODS test_no_match_found3 FOR TESTING.
ENDCLASS.

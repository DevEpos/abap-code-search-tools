...
CLASS ltcl_abap_unit DEFINITION FINAL
  ...
  PRIVATE SECTION.
    ...
    METHODS test_tranform_to_string   FOR TESTING.
    METHODS test_adjust_line_endings  FOR TESTING.
    METHODS test_adjust_line_endings2 FOR TESTING.
    METHODS test_adjust_line_endings3 FOR TESTING.
ENDCLASS.


CLASS ltcl_abap_unit IMPLEMENTATION.
  METHOD test_tranform_to_string.
    DATA(source_table) = VALUE string_table( ( `This is the first line` )
                                             ( `and another line` )
                                             ( ` now we are done` ) ).
    ...
    zcl_adcoset_string_util=>transform_to_string( EXPORTING source_table = source_table
                                                            line_feed    = line_feed
                                                  IMPORTING source_text  = DATA(source_text)
                                                            indexes      = DATA(indexes) ).

    cl_abap_unit_assert=>assert_not_initial( act = source_text
